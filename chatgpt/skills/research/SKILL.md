...
ChatGPT's Deep Research flow asks 1–N follow-up questions (scope, timeframe, sources, depth) before kicking off. **Default: auto-answer so the run is fire-and-forget.**
...
### 2. Confirm the MCP is in _attach_ mode, not _launch_ mode

The MCP must be in **attach mode** (`--autoConnect` or `--browser-url`, driving the user's real Chrome), not **launch mode** (its own Chrome with a throwaway profile at `~/.cache/chrome-devtools-mcp/chrome-profile-stable`, no logins). Launch mode is also more likely to be blocked by Cloudflare on chatgpt.com. See `docs/browser-setup.md`.

Sanity-check before navigating to chatgpt.com: navigate to `chrome://version` and read `Profile Path` from the accessibility snapshot. If it looks like the throwaway cache dir, stop and point the user at `docs/browser-setup.md`; suggest `claude mcp list` (or the equivalent for their harness) to confirm current flags.
...
If Deep research is still missing, stop and report what menu entries you _did_ see. The user's account tier may not include Deep Research, or OpenAI may have moved it elsewhere entirely.
...
- **`+` / Tools menu not found** → stop at step 6; report what controls you _did_ see. The composer may be in a state the skill doesn't know about, or the user's account tier may be hiding the menu.
